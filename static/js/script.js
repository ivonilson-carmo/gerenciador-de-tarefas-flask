    ...
    $.ajax({
        ...
        success: function (response){
            if (response.error){
                ...
                const msg = `Completada em: ${response.completed_at}`
                event.target.parentNode.parentNode.querySelector('p.completed-at').innerText = msg;
    ...
    $.ajax({
        ...
        success: function(response) {
            if(response.error){
                ...
                <p class="title-task fs-6 col-12 text-break mb-1"> ${response.task} </p>
                <p class="created-at col-sm-12 col-md-6 mb-0"> Criado em: ${response.created_at} </p>
                <p class="completed-at col-sm-12 col-md-6 mb-0"> </p>
